-- Aula 9: versioned only; review/test in isolated Supabase before any production application.
-- Scope: qa_bugs. Mission entries require their own authorized workflow bridge.
-- Existing status values remain unchanged; no data backfill or destructive operation.
...
create policy qa_bug_status_events_select on public.qa_bug_status_events
  for select to authenticated using (
    exists (select 1 from public.qa_bugs b where b.id = bug_id
      and public.qa_can_view(b.author_id, b.group_id, auth.uid()))
  );

-- Developers can update assigned tickets in their own group only. Existing policies
-- remain active; the trigger below is the authoritative status-transition guard.
drop policy if exists qa_bugs_assigned_developer_update on public.qa_bugs;
create policy qa_bugs_assigned_developer_update on public.qa_bugs
  for update to authenticated
  using (assignee_id = auth.uid() and group_id is not null
    and public.is_group_member(group_id, auth.uid()))
  with check (assignee_id = auth.uid() and group_id is not null
    and public.is_group_member(group_id, auth.uid()));

create or replace function public.qa_guard_bug_status()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  -- Prevent reassignment, scope/ownership changes and silent rewriting of status history.
  ...
  v_tester := v_actor = old.author_id and old.assignee_id is not null
    and old.assignee_id <> v_actor;
  v_developer := v_actor = old.assignee_id and old.author_id <> v_actor;
  -- Retests must be written atomically by qa_register_retest; that RPC updates
  -- status before inserting the retest, within the same transaction.
  v_allowed := case
    ...
    when old.status = 'pronto_reteste' and new.status in ('resolvido','reaberto') then v_tester
...
-- Remaining gate before deployment: prohibit direct tester status changes to
-- resolved/reopened except via atomic RPC; integrate UI and mission bugs;
-- validate policies, triggers and legacy records against isolated database.
